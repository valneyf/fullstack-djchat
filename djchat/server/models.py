...
class Category(models.Model):
    ...
    def save(self, *args, **kwargs):
        if self.id:
            previous = get_object_or_404(Category, id=self.id)
            if previous != self.icon:
                previous.icon.delete(save=False)
...
class Channel(models.Model):
    ...
    def save(self, *args, **kwargs):
        if self.id:
            previous = get_object_or_404(Server, id=self.id)

            if previous.icon != self.icon:
                previous.icon.delete(save=False)

            if previous.banner != self.banner:
                previous.banner.delete(save=False)
